...
class Reader:
    ...
    def char_encoded(self, char):
        ...
        asci = str(ord(char))
        ctrl_sum = self.calc_control_sum(asci)
        # counting how many marks does ascii_char have to check how many 0 to add
        if len(asci) == 1:
            asci = "00" + asci
        elif len(asci) == 2:
            asci = "0" + asci
        # getting random ISN 10 number range (we checked that before,
        # explained in documentation)
        rand = str(self.get_random(1000020914, 4294873279))
        # adding encoded to ISN
        for i, x in enumerate(asci):
            step = (i+1) * 3
            rand = "".join([rand[:step-1], x, rand[step:]])

        new_ctrl_sum = self.new_ctrl_sum(asci)
    ...
    # 3268-63998 is range of ports
        rand = str(self.get_random(32768, 63998))
